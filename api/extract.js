// Server-side URL content extractor (Vercel Edge Function).
// Replaces the client-side api.allorigins.win dependency with a first-party
// fetch so CORS is handled server-side and there is no third-party SPOF.
// GET /api/extract?url=<encoded-url>
//
// Self-contained on purpose: this used to import from functions/api/extract.js,
// and deleting that folder broke every production deploy for hours because
// `astro build` does not bundle api/, so nothing failed locally.
